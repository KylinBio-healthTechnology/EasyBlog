	...
	<div class="container">
		...
		<div class="well">
			<table class="table">
				...
				<c:forEach items="${selManage }" var="art">									
					<tr>
						<td><a href="<%=basePath%>ArtCom?userId=${user.id }&artId=${art.id}">${art.title }</a></td>
		...
		<div>
			...
			<c:if test="${curPage>1 }">
				...
				<li class="previous"><a
					href="<%=basePath%>ArticleManage&userId=${user.id }&curPage=${curPage-1}">&larr;
						上一页</a></li>
				
            </c:if>
            <c:if test="${curPage<totalPages }">				
				<li class="next"><a
					href="<%=basePath%>ArticleManage&userId=${user.id }&curPage=${curPage+1}">下一页
						&rarr;</a></li>
			</c:if>
			</ul>
		</div>
