...
<nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
    <div class="container">
        <div class="navbar-header">
            <a class="navbar-brand" href="<%=basePath%>SysCategoryList">SSM Blog System</a>
        </div>
        ...
        <div class="collapse navbar-collapse navbar-ex1-collapse">
            ...
            <ul class="nav navbar-nav">
                <li><a href="<%=basePath %>AdminLogin.jsp" target="_blank">管理员登录后台系统</a></li>
            </ul>
...
<div class="container">
    <div class="well">
       ...
       <%--  <c:choose>
             <c:when test="${selCom!=null }">
                 <c:forEach items="${selCom }" var="com">
                     <tr>
                         <td>${com.conment }</td>
                         <td>${com.user.id }</td>
                         <td>${com.article.title}</td>
                         <td>${com.time }</td>
                         <td><a href="<%=basePath%>deleteComment?cmtId=${com.id}&userId=${user.id}">
                             <i class="glyphicon glyphicon-remove"></i></a></td>
                     </tr>
    ...
    <div>
        ...
        <ul class="pager">
            <c:if test="${curPage>1 }">
                <li class="previous"><a href="<%=basePath%>CommentManage&userId=${user.id }&curPage=${curPage-1}">&larr;
                    上一页</a></li>
            </c:if>
            <c:if test="${curPage<totalPages }">
                <li class="next"><a href="<%=basePath%>CommentManage&userId=${user.id }&curPage=${curPage+1}">下一页
                    &rarr;</a></li>
            </c:if>
        </ul>
    </div>
